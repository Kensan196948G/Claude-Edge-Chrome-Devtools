...
#     代替: scripts\main\Claude-DevTools.ps1 -Browser edge
...
# ⚠️  非推奨警告
Write-Warning "⚠️  Claude-EdgeDevTools.ps1 は非推奨です。代わりに Claude-DevTools.ps1 -Browser edge を使用してください。(MIGRATION.md 参照)"
...
# Statusline の有効化状態と base64 エンコード
$statuslineEnabled = $Config.statusline.enabled -eq $true
$encodedStatusline = $null
$statuslineScriptPath = Join-Path (Split-Path $PSScriptRoot -Parent) "statusline.sh"
if ($statuslineEnabled -and (Test-Path $statuslineScriptPath)) {
    $statuslineContent = Get-Content -Path $statuslineScriptPath -Raw -Encoding UTF8
...
# 外部テンプレートからrun-claude.shを生成（bash heredoc問題を回避）
$templatePath = Join-Path $PSScriptRoot '..\..\scripts\templates\run-claude-edge-template.sh'
$initPromptPath = Join-Path $PSScriptRoot '..\..\scripts\templates\init-prompt-ja.txt'

if (-not (Test-Path $templatePath)) {
    throw "run-claudeテンプレートファイルが見つかりません: $templatePath"
}
$RunClaude = Get-Content -Path $templatePath -Raw -Encoding UTF8
...
if ($Config.claudeCode -and $Config.claudeCode.env) {
    foreach ($key in $Config.claudeCode.env.PSObject.Properties.Name) {
        $val = $Config.claudeCode.env.$key
        $envExports += "export $key='$val'`n"
    }
}
$RunClaude = $RunClaude -replace "__ENV_EXPORTS__", $envExports.TrimEnd()

# INIT_PROMPT を外部ファイルから読み込み
if (Test-Path $initPromptPath) {
    $initPromptContent = Get-Content -Path $initPromptPath -Raw -Encoding UTF8
    $RunClaude = $RunClaude.Replace('__INIT_PROMPT__', $initPromptContent.TrimEnd())
} else {
    Write-Warning "init-prompt-ja.txtが見つかりません。デフォルトプロンプトを使用します"
    $RunClaude = $RunClaude.Replace('__INIT_PROMPT__', 'Claude Code が起動しました。何かお手伝いできますか？')
}

# CRLF → LF 変換
$RunClaude = $RunClaude -replace "`r`n", "`n" -replace "`r", "`n"

# Windowsのプロジェクトパスにrun-claude.shを書き込む
[System.IO.File]::WriteAllText($RunClaudePath, $RunClaude, [System.Text.Encoding]::UTF8)
Write-Host "✅ run-claude.sh を生成しました: $RunClaudePath" -ForegroundColor Green

if ($statuslineEnabled) {
    if ($null -ne $encodedStatusline) {
        # settings.json を生成
        $SettingsJson = @{
            statusLine = @{
                type = "command"
