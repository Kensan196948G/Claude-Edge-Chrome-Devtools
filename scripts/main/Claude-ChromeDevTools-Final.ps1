...
# INIT_PROMPT変数は削除（CLAUDE.mdから自動読み込み）
...
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
echo "💡 ヒント: CLAUDE.mdに詳細な指示があります"
echo ""

while true; do
  # Claude Codeを起動（初期プロンプトはCLAUDE.mdから自動読み込み）
...
if ($SelectedProjects.Count -gt 1) {
    ...
    foreach ($proj in $SelectedProjects) {
        ...
        $Job = Start-Job -ScriptBlock {
            ...
            ssh -tt -o ControlMaster=no -o ControlPath=none -R "${Port}:127.0.0.1:${Port}" $LinuxHost "cd '${LinuxBase}/${ProjectName}' && ./run-claude.sh"
    ...
    # SSH接続してrun-claude.shを実行（-tt で強制的にpseudo-ttyを割り当て）
    $EscapedProjectName = Escape-SSHArgument $ProjectName
    $EscapedLinuxBase = Escape-SSHArgument $LinuxBase
    ssh -tt -o ControlMaster=no -o ControlPath=none -R "${DevToolsPort}:127.0.0.1:${DevToolsPort}" $LinuxHost "cd $EscapedLinuxBase/$EscapedProjectName && ./run-claude.sh"
}
